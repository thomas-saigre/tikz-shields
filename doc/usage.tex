...
\mintex{\usepackage[labelColor=red]{tikz-shields}}.
...
\mintex{\def\checkmark{\tikz\fill[scale=0.4] (0,.35) -- (.25,0) -- (1,.7) -- (.25,.15) -- cycle;}}).
...
To avoid them, you might need to add the option \mintex{font=\normalfont}, as in the two latter examples here.
...
\begin{itemize}
    ...
    \item \mintex{{<repo path>}} (required): Path to the repository.
\end{itemize}
...
\begin{itemize}
    ...
    \item \mintex{{<license name>}} (required): Name of the license to display.
\end{itemize}
...
\begin{itemize}
    ...
    \item \mintex{{<id>}} (required): Number of identification of the publication.
\end{itemize}
